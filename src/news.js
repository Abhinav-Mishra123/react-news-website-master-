    ...
    async componentDidMount() {
      ...
      let arr = data.articles.map((p) => {
        return (
          ...
          <div class="p-2 main-container" style={{boxSizing: "border-box"}}>  
          {/* <!--Card 1--> */}
          <div class="max-w-sm rounded overflow-hidden shadow-lg">
            <img class="w-full imgnews" src={p.urlToImage} alt="news"/>
            <div class="px-6 py-4">
            ...
            <div class="px-6 pt-4 pb-2">
              <span class="inline-block bg-gray-200 rounded-full px-3 py-1 text-sm font-semibold text-black-700 mr-2 mb-2">{p.source.name}</span>
              <span class="inline-block bg-gray-200 rounded-full px-3 py-1 text-sm font-semibold text-black-700 mr-2 mb-2">{p.publishedAt}</span>
              <span class="inline-block bg-gray-200 rounded-full px-3 py-1 text-sm font-semibold text-gray-700 mr-2 mb-2">Latest News</span>
            </div>
          </div>
        </div>
        );
